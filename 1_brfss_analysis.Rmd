...
cdc %>% summarise(mean_height = mean(height), sd_x =(sd(height)), mean_age = mean(age), sd_x = sd(age))
...
no_excellent_health_ <- cdc %>% 
  filter(genhlth == "excellent") %>% 
  nrow()

no_excellent_health_/nrow(cdc)
```
...
```{r exercise06}
cdc_with_diff_weight <- cdc %>% 
  mutate(diff_weight = wtdesire - weight)
```
...
#wdiff is an interger/numeric
#0 = no weight difference
